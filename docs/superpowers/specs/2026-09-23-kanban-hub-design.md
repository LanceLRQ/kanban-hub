...
**进度**：已完成数 ÷（总数 − 已取消数）。任务自己的清单单独计算完成度。
...
统一前缀 `/api/v1`，请求和响应都用 JSON，并用 `packages/core` 里的 zod schema 校验。`kh` 每次请求都带 `X-KH-Version` 头，服务端判断不兼容时返回 426；`/api/v1` 接口的响应都带 `X-KH-Version`，标明服务端版本。令牌请求可以另外带 `X-KH-Agent`（不超过 50 个字符）标识自己，会写进对应事件的操作者。
