...
/** 容器在命令输出里的简短标识：杂项容器固定显示 misc，其余容器用编号，没编号时给出退化提示 */
export function containerLabel(container: Pick<Container, "kind" | "code">): string {
  if (container.kind === "misc") return "misc";
  return container.code ?? "(无编号)";
}

/** 命令行展示可空字段的变化时，把 null/空串统一显示成 (空) */
export function displayValue(v: string | null): string {
  return v === null || v === "" ? "(空)" : v;
}
...
export function assertHasSetOption(opts: SetOptionsInput): void {
  ...
  if (!has) throw new CliError(EXIT.USAGE, "kh task set 至少需要提供一个要修改的选项");
}

/** --status suspended 却没给 --reason 时本地拦截（用法错误 2），不发请求；核对用服务端 transitionTask 负责 */
export function assertSuspendReason(status: string | undefined, reason: string | undefined): void {
  if (status === "suspended" && reason === undefined) {
...
/** set/human/check/checklist 共用的前置步骤：找仓库、登录、拉项目详情、按引用解析出任务 */
async function loadTaskEditContext(ctx: CliContext, cmd: Command, taskRef: string): Promise<TaskEditContext> {
...
async function runAdd(ctx: CliContext, containerRef: string, title: string, opts: AddOptions, cmd: Command): Promise<void> {
  ...
  if (opts.due !== undefined) candidate.dueDate = opts.due;
  if (opts.doc.length > 0) candidate.docRefs = opts.doc.map((d) => toRepoPath(repo.root, ctx.cwd, d));
  ...
  const codePart = task.code !== null ? `（${containerLabel(container)}/${task.code}）` : "";
...
async function runSet(ctx: CliContext, taskRef: string, opts: SetOptions, cmd: Command): Promise<void> {
  ...
  const { repo, client, detail, task } = await loadTaskEditContext(ctx, cmd, taskRef);
  ...
  const due = parseNullableOption(opts.due);
  ...
  if (opts.doc.length > 0) {
    const added = opts.doc.map((d) => toRepoPath(repo.root, ctx.cwd, d));
    docRefs = dedupePaths([...task.docRefs, ...added]);
  ...
  if (status !== undefined) changes.push(`状态 ${TASK_STATUS_LABELS[task.status]} → ${TASK_STATUS_LABELS[updated.status]}`);
  if (opts.reason !== undefined) changes.push(`挂起原因 ${displayValue(task.suspendReason)} → ${displayValue(updated.suspendReason)}`);
  if (opts.note !== undefined) changes.push(`备注 ${displayValue(task.note)} → ${displayValue(updated.note)}`);
  if (opts.title !== undefined) changes.push(`标题 ${displayValue(task.title)} → ${displayValue(updated.title)}`);
  if (code !== undefined) changes.push(`编号 ${displayValue(task.code)} → ${displayValue(updated.code)}`);
  if (group !== undefined) changes.push(`分组 ${displayValue(task.group)} → ${displayValue(updated.group)}`);
  if (due !== undefined) changes.push(`截止日期 ${displayValue(task.dueDate)} → ${displayValue(updated.dueDate)}`);
  if (targetContainer !== undefined) {
    const from = detail.board.containers.find((c) => c.id === task.containerId);
    changes.push(`所属容器 ${from ? containerLabel(from) : task.containerId} → ${containerLabel(targetContainer)}`);
  }
  if (docRefs !== undefined) changes.push(`关联文档 ${task.docRefs.length} 项 → ${updated.docRefs.length} 项`);
...
async function runHuman(ctx: CliContext, taskRef: string, opts: HumanOptionsInput, cmd: Command): Promise<void> {
  ...
  const from = task.human ? `${HUMAN_KIND_LABELS[task.human.kind]}：${task.human.note}` : "(无)";
  const to = updated.human ? `${HUMAN_KIND_LABELS[updated.human.kind]}：${updated.human.note}` : "(无)";
